...
func EncodeMessage(msg interface{}, ctx cellnetEx.ContextSet) (data []byte, meta *cellnetEx.MessageMeta, err error) {
	...
	// 获取消息元信息
	meta = cellnetEx.MessageMetaByMsg(msg)
	if meta == nil {
		return nil, nil, cellnetEx.NewErrorContext("msg not exists", msg)
	}
...
func DecodeMessage(msgid int, data []byte) (interface{}, *cellnetEx.MessageMeta, error) {
	...
	// 获取消息元信息
	meta := cellnetEx.MessageMetaByID(msgid)

	// 消息没有注册
	if meta == nil {
		return nil, nil, cellnetEx.NewErrorContext("msg not exists", msgid)
	}
