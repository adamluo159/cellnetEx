...
func client() {
	...
	p.(cellnetEx.WSConnector).SetReconnectDuration(time.Second)

	proc.BindProcessorHandler(p, "gorillaws.ltv", func(ev cellnetEx.Event) {
...
func server() {
	...
	proc.BindProcessorHandler(p, "gorillaws.ltv", func(ev cellnetEx.Event) {
		...
		case *TestEchoACK:
			...
			val, exist := ev.Session().(cellnetEx.ContextSet).GetContext("request")
			if exist {
