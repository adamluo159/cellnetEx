...
func (self *tcpSession) Peer() cellnetEx.Peer {
...
func (self *tcpSession) recvLoop() {
	...
	if i, ok := self.Peer().(cellnetEx.PeerCaptureIOPanic); ok {
	...
	for self.Conn() != nil {
		...
		if err != nil {
			...
			// 标记为手动关闭原因
			closedMsg := &cellnetEx.SessionClosed{}
			if self.IsManualClosed() {
				closedMsg.Reason = cellnetEx.CloseReason_Manual
			}

			self.ProcEvent(&cellnetEx.RecvMsgEvent{Ses: self, Msg: closedMsg})
			break
		}
...
func newSession(conn net.Conn, p cellnetEx.Peer, endNotify func()) *tcpSession {
	self := &tcpSession{
		conn:       conn,
		endNotify:  endNotify,
