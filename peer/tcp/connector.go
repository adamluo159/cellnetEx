...
func (self *tcpConnector) Start() cellnetEx.Peer {
...
func (self *tcpConnector) Session() cellnetEx.Session {
	...
	for {
		...
		if err != nil {
			...
			if self.ReconnectDuration() == 0 || self.IsStopping() {
				...
				self.ProcEvent(&cellnetEx.RecvMsgEvent{
					Ses: self.defaultSes,
					Msg: &cellnetEx.SessionConnectError{},
				})
				break
			}
