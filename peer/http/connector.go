...
func (self *httpConnector) Start() cellnetEx.Peer {
...
func getCodec(codecName string) cellnetEx.Codec {
...
func (self *httpConnector) Request(method, path string, param *cellnetEx.HTTPRequest) error {
	...
	if log.IsDebugEnabled() {
		log.Debugf("#http.send(%s) '%s' %s | Message(%s) %s",
			...
			cellnetEx.MessageToString(param.REQMsg))
	}
	...
	if log.IsDebugEnabled() {
		log.Debugf("#http.recv(%s) '%s' %s | [%d] Message(%s) %s",
			...
			cellnetEx.MessageToString(param.ACKMsg))
	}
