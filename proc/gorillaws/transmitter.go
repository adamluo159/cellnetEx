...
func (WSMessageTransmitter) OnRecvMessage(ses cellnetEx.Session) (msg interface{}, err error) {
...
func (WSMessageTransmitter) OnSendMessage(ses cellnetEx.Session, msg interface{}) error {
	...
	switch m := msg.(type) {
	case *cellnetEx.RawPacket: // 发裸包
		msgData = m.MsgData
		msgID = m.MsgID
	default: // 发普通编码包
		var err error
		var meta *cellnetEx.MessageMeta
